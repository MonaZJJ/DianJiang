...
<script>
    import { userQrCode,jsSdkShare } from "@/api/member";
        ...
        watch: {
            '$route': {
                handler: function (to, from) {
                    ...
                    //转发的链接（或者从个人中心跳转过来）和缓存的会员uid比较
                    // 二者一样 显示当前页面；不一样 跳转登录页面
                    if (that.shareUid==localStorage.getItem("uid")) {
                        userQrCode({
                            "UId": that.shareUid
                        }).then(response => {
                            ...
                            that.pageUrl = process.env.VUE_APP_BASE_API_SHARE + '/myCode?shareUid=' + response.Data.UId;
