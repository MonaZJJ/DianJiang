...
var DepartmentPower = {
    init: function () {
        ...
        $('body').on('click', '#save', function () {
            Common.confirmDialog("确定修改该部门的权限吗？", function () {
                var checkPermissionIds = DepartmentPower.getCheckPermissions();
                DepartmentPower.changePowerSetting(checkPermissionIds);
            })
        });
    ...
    //查询权限列表
    listPower: function () {
        var methodName = "/admin/AdminPermissionsList";
    ...
    //设置权限
    changePowerSetting: function (permissionIds) {
        var methodName = "/admin/AdminSetPermissions";
        var departmentId = Common.getUrlParam('departmentId');
        var data = {
            "DepartmentId": departmentId,
            "ActionIds": permissionIds
        };
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
