var jurisdictionOperation = {
    init: function () {
        //获取管理员列表
        jurisdictionOperation.initBootstrapTable();
        //点击确认添加管理员模态框
        $('#add_manage_confirm').click(function () {

        });
        //获取它的总的部门数目和类别
        jurisdictionOperation.getDepartmentList();

        //点击查询按钮
        $('#searchBtn').click(function () {
            var data = {
                "UserName": $('#manage_Name').val(),
                "DepartmentId": $('#departmentCate').find("option:selected").val()
            }
            jurisdictionOperation.projectQuery(data);
        });

        //点击添加管理员
        $('#add_manage_confirm').click(function () {
            jurisdictionOperation.addManager();
        });
        ...
        $('#manageListTb').on('click', '.status_delete', function () {
            ...
            Common.confirmDialog('是否删除该管理员?', function () {
                jurisdictionOperation.deleteManager(dataId);
            })
    ...
    getDepartmentList: function () {
        var methodName = "/admin/AdminAllDepartmentList";
        var data = {};
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
                var render = template.compile(jurisdictionOperation.cateTpl);
    ...
    addManager: function (info) {
        ...
        var departmentId = $('#which_department').find("option:selected").val();
        if (username != '' && password != '' && confirmPassword != '' && departmentId != '') {
    ...
    initBootstrapTable: function () {
        $('#manageListTb').bootstrapTable({
            ...
            queryParams: jurisdictionOperation.queryParams, //参数
            queryParamsType: "limit", //参数格式,发送标准的RESTFul类型的参数请求
            toolbar: "#toolbar", //设置工具栏的Id或者class
            responseHandler: jurisdictionOperation.responseHandler,
            columns: [
                {
                    field: 'UserName',
                    title: '用户名',
                ...
                },
                {
                    ...
                    formatter: function (value, row, index) {
                        ...
                        var html = '<a href="/jurisdiction/jurisdictionEdit?id=' + value + '" style="color: #4E70FB;cursor: pointer;" class="edit_manage" data-id="' + value + '" >编辑</a>' +
