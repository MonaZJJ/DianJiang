...
var ProductList = {
    ...
    init: function () {
        if (localStorage.getItem('PageIndex')) {
            ...
            //商品分类
            ProductList.getProductCategory();
            setTimeout(function () {
    ...
    adminEditProductDisplayOrder: function (PId, DisplayOrder) {
        var methodName = "/Product/AdminEditProductDisplayOrder";
        var data = {
            "PId": PId,
    ...
    // 后台设置商品是否热销
    adminSetHotProduct: function (PId,state) {
        var methodName = "/product/AdminSetHotProduct";
        var data = {
            PId: PId,
            "Type": state
        };
    ...
    // 获取商品品牌列表
    getAdminBrandList: function () {
        var methodName = "/brand/AdminBrandList";
        var data = {
        };
        SignRequest.set(methodName, data, function (data) {
            if (data.Code == "100") {
                var render = template.compile(ProductList.styleTpl);
                var html = render(data.Data);
