...
`,
    ...
    init: function () {
        ...
        //获取信息
        addFreightTemplate.adminTemplatesInfo();
        //省改变时
        ...
        $('body').on('click','#addFree',function(){
            ...
            var four = $("#onNumberfour").val()||0;
            var five = $("#onNumberfive").val()||0;
            var html = `<tr><td><a href="javascript:void(0);" class="exit-area">编辑</a><div class="area-group"><p>未添加地区</p></div></td><td><input type="text" class="form-control input-xs" value="`+one+`"></td><td><input type="text" class="form-control input-xs" value="`+two+`"></td><td><input type="text" class="form-control input-xs" value="`+three+`"></td><td><input type="text" class="form-control input-xs" value="`+four+`"></td><td><input type="text" class="form-control input-xs" value="`+five+`"></td><td><span class="btn-a"><a href="javascript:void(0);" name="delContent">删除</a></span></td></tr>`
        ...
        $('body').on('click','#submitBtn',function(){
            addFreightTemplate.adminAddTemplates();
        })
    },
    //编辑运费模板接口
    adminAddTemplates:function(){
        var methodName = "/Templates/AdminEditTemplates";
        ...
        var five = $("#onNumberfive").val()||0;
        var data = {
            "TemplateId": Common.getUrlParam('id'),
            ...
            "MinPrice": five,
            "IsRegionFree":$('input:radio[name="state"]:checked').val() == 0 ? true: false,
            "State": $('input:radio[name="status"]:checked').val()
        };
        SignRequest.set(methodName, data, function (data) {
            console.log(data)
            if (data.Code == "100") {
                Common.showSuccessMsg('编辑成功',function(){
                    location.href = '/platform/freightTemplate'
    ...
    //后台运费模板信息
    adminTemplatesInfo:function(){
        var methodName = "/Templates/AdminTemplatesInfo";
        var data = {
            "TemplateId": Common.getUrlParam('id')
        };
        SignRequest.set(methodName, data, function (data) {
            console.log(data)
            if (data.Code == "100") {
                $("#templateName").val( data.Data.AdminTemplatesInfo.ShippingTemplates.TemplateName);
                if(data.Data.AdminTemplatesInfo.ShippingTemplates.IsRegionFree == true ){
                    $("#stateYes").iCheck('check');
                }else{
                    $("#stateNo").iCheck('check');
                }
                if( data.Data.AdminTemplatesInfo.ShippingTemplates.State == 1 ){
                    $("#statusYes").iCheck('check');
                }else{
                    $("#statusNo").iCheck('check');
                }
                var type = data.Data.AdminTemplatesInfo.ShippingTemplates.ValuationMethod;
                if(type == "4"){
                    $('#one_num1').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.SPrice)
                    $('#one_num2').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.SFreight)
                    $('#one_num3').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.StartPrice)
                    $('#one_num4').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.EndPrice)
                    $('#one_num5').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.Freight)
                    $('#one_num6').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.WithFree)
                    $('#onpercentage').iCheck('check');
                    $("#regionFreight,#addCityFreight,#formatBox,#freightBox").hide();
                    $('#PercentageBox').show();
                }
                if(type == "5"){
                    $('#two_num1').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.SPrice)
                    $('#two_num2').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.SFreight)
                    $('#two_num3').val(data.Data.AdminTemplatesInfo.ShippingPriceInfo.WithFree)
                    $("#regionFreight,#addCityFreight,#formatBox,#PercentageBox").hide();
                    $('#freightBox').show();
                    $('#onFixedFreight').iCheck('check');
                }
                // if(type=="1"){
                //     $('#onpercentage').iCheck('check');
                //     $("#regionFreight,#addCityFreight,#formatBox,#freightBox").hide();
                ...
                //     $('.nofloat[name="ValuationUnit"]').html('kg')
                //     $('.nofloat[name="ValuationUnitDesc"]').html('重')
                //     $('#onWeight').iCheck('check')
                //     $("#regionFreight,#addCityFreight,#formatBox").show();
                //     $('#freightBox,#PercentageBox').hide();
                // }else if(type=="3"){
                //     $('.changeText').html('m<sup>3</sup>')
                //     $('.nofloat[name="ValuationUnit"]').html('m<sup>3</sup>')
                //     $('.nofloat[name="ValuationUnitDesc"]').html('体积')
                //     $('#onVolume').iCheck('check')
                //     $("#regionFreight,#addCityFreight,#formatBox").show();
                //     $('#freightBox,#PercentageBox').hide();
                // }else


                // $("#onNumberone").val(data.Data.AdminTemplatesInfo.ShippingTemplates.DefaultNumber);
                // $("#onNumbertwo").val(data.Data.AdminTemplatesInfo.ShippingTemplates.Price);
                // $("#onNumberthree").val(data.Data.AdminTemplatesInfo.ShippingTemplates.AddNumber);
                // $("#onNumberfour").val(data.Data.AdminTemplatesInfo.ShippingTemplates.AddPrice);
                // $("#onNumberfive").val(data.Data.AdminTemplatesInfo.ShippingTemplates.MinPrice);
                // $('#templateName').val(data.Data.AdminTemplatesInfo.ShippingTemplates.TemplateName);

                //     if(data.Data.AdminTemplatesInfo.ShippingRegionsGroupsList.length>0){
                //     var list = data.Data.AdminTemplatesInfo.ShippingRegionsGroupsList;
                //     var html = "";
                //     for(var i=0;i<list.length;i++){
                //         var one = list[i].DefaultNumber;
                //         var two = list[i].Price;
                //         var three = list[i].AddNumber;
                //         var four = list[i].AddPrice;
                //         var five = list[i].MinPrice;
                //         var name = list[i].ShippingRegionsList[0].Name;
                //         var pid = list[i].ShippingRegionsList[0].ProvinceId;
                //         var cid = list[i].ShippingRegionsList[0].CityId;
                //         var aid = list[i].ShippingRegionsList[0].RegionId;
                //
                //         html += `<tr><td><a href="javascript:void(0);" class="exit-area">编辑</a><div class="area-group"><p data-pid="`+pid+`" data-cid="`+cid+`" data-aid="`+aid+`">`+name+`</p></div></td><td><input type="text" class="form-control input-xs" name="one" value="`+one+`"></td><td><input type="text" name="two" class="form-control input-xs" value="`+two+`"></td><td><input type="text "name="three" class="form-control input-xs" value="`+three+`"></td><td><input type="text" name="four" class="form-control input-xs" value="`+four+`"></td><td><input type="text" name="five" class="form-control input-xs" value="`+five+`"></td><td><span class="btn-a"><a href="javascript:void(0);" name="delContent">删除</a></span></td></tr>`
                //
                //     }
                //     $('#regionFreight tbody').html(html)
                // }
            } else {
                Common.showErrorMsg(data.Message);
            }
